...
class LLP_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'label': label, 'video_name': video_name}
...
def get_segment_pseudo_labels(logits_audio_text, thresholds, labels, device):
    ...
    with torch.no_grad():
        ...
        Pa = torch.zeros(10, 28).long().to(device)
...
def audio_label_elaboration(model, data_loader, event_captions, audio_path, thresholds, save_path, device, print_progress):
    ...
    text_embeds = get_text_embedding(model, event_captions)    
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            prompt_embeds = get_text_embedding(model, descrption[video_name])  
            prompt_embeds = torch.from_numpy(prompt_embeds).to(device)
            audio_embeds = get_audio_embedding(model, video_name, audio_path)     
...
def get_audio_feature_maps(my_model, data_loader, audio_path, save_path, print_progress):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(data_loader):
            ...
            audio_feat_maps = my_model.get_audio_feature_maps([temp_dict])  
            if len(descrption[video_name]) > 300:
                descrption[video_name] = descrption[video_name][:300]
            text_embeds = get_text_embedding(model, descrption[video_name])  
            np.save(os.path.join(save_path, video_name[:11] + '_text.npy'), text_embeds)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--audio_dir", type=str, default='data/AVE', help="the directory where .mp4 or .wav files are saved")
    ...
    whole_dataset = LLP_dataset(label_txt=args.label_all_dataset)
    ...
    '''
    event_captions = ['Church bell', 'Male speech, man speaking', 'Bark', 'Fixed-wing aircraft, airplane', 'Race car, auto racing', 'Female speech, woman speaking',
              'Helicopter', 'Violin, fiddle', 'Flute', 'Ukulele', 'Frying (food)',
              'Truck', 'Shofar', 'Motorcycle', 'Acoustic guitar',
              'Train horn', 'Clock', 'Banjo', 'Goat', 'Baby cry, infant cry',
              'Bus', 'Chainsaw', 'Cat', 'Horse',
