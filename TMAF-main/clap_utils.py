...
#用于提取音频特征图
    def get_audio_feature_maps(self, data):
        ...
        keys = data[0].keys()
        for k in keys:
            input_dict[k] = torch.cat([d[k].unsqueeze(0) for d in data], dim=0).to(self.device) #取出所有样本的特征，并将其按批次维度堆叠，得到输入特征矩阵
...
#时间序列通道注意力： 使用 tscam_conv 层对音频特征进行处理，可能包含时间序列上的通道注意力机制，用于提升特征的表达能力
